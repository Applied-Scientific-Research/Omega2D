...
public:
  ...
  void find_vels( const std::array<double,Dimensions>&,
                  ...
                  std::vector<Collection>&,
                  std::vector<Collection>&,
                  const solution_t _results = velonly,
...
void Convection<S,A,I>::find_vels(const std::array<double,Dimensions>& _fs,
  ...
  InfluenceVisitor<A> visitor = {SolnType(_results), conv_env};
  ...
  for (auto &targ : _targets) {
    ...
    std::cout << "  Solving" << SolnType(_results).to_string() << " on" << to_string(targ) << std::endl;
